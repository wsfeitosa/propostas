...
class Breakdown extends CI_Model {
	...
	public function formatarBreakDown()
	{
		...
		if( ! empty($row->transit_via_x_via_adicional) )
		{

			if( $this->tarifario->getSentido() == "EXP" )
			{
				$transit_loading_x_discharge = $this->tarifario->getRota()->getPortoEmbarque()->getNome() . " / " .
										   	   $this->tarifario->getRota()->getPortoDesembarque()->getNome() . ": " .
										   	   $row->transit_loading_x_via . " DIAS APROX. | FREQUENCIA: " .
				...
				$transit_via_x_via_adicional = $this->tarifario->getRota()->getPortoDesembarque()->getNome() . " / " .
										   	   $this->tarifario->getRota()->getPortoViaAdicional()->getNome() . ": " .
										   	   $row->transit_via_x_via_adicional . " DIAS APROX. | FREQUENCIA: " .
										   	   $this->frequencias[$row->id_frequencia_via_x_via_adicional] . " | ON BOARD";

				$transit_dicharge_x_destination = $this->tarifario->getRota()->getPortoViaAdicional()->getNome() . " / " .
										      	  $this->tarifario->getRota()->getPortoFinal()->getNome() . ": " .
										      	  $row->transit_via_adicional_x_place_delivery . " DIAS APROX. | FREQUENCIA: " .
										      	  $this->frequencias[$row->id_frequencia_via_adicional_x_place_delivery];						   	   
			}	
			else
			{
				$transit_origem_x_loading = $this->tarifario->getRota()->getPortoOrigem()->getNome() . " / " .
											$this->tarifario->getRota()->getPortoViaAdicional()->getNome() . ": ". 
											$row->transit_receipt_x_loading . " DIAS APROX. | FREQUENCIA: " . 
											$this->frequencias[$row->id_frequencia_via_x_via_adicional];

				$transit_via_x_via_adicional = $this->tarifario->getRota()->getPortoViaAdicional()->getNome() . " / " .
										   	   $this->tarifario->getRota()->getPortoEmbarque()->getNome() . ": " .
										   	   $row->transit_via_x_via_adicional . " DIAS APROX. | FREQUENCIA: " .
										   	   $this->frequencias[$row->id_frequencia_via_adicional_x_place_delivery] . " | ON BOARD";
			}	
