...
class Altera_Propostas extends CI_Model {
	...
	public function AlterarPropostasRetroativas()
	{
		 ...
		 */
		if( $this->router->class == "adicionais" && $this->router->method == "autorizar_desbloqueio" )
		{
			$handle = fopen("/var/www/html/allink/post.txt", "w+");
			fwrite($handle, $this->input->post('alterar_retroativos-'.$this->input->post('id_acordo')));
			fclose($handle);
			
			if( $this->input->post('alterar_retroativos-'.$this->input->post('id_acordo')) != "S" )
			{
				return false;
			}
